...
from supervisor.http_client import HTTPClient
...
class Options:
    ...
    def process_config_file(self, do_usage):
        ...
        try:
            ...
            # self.read_config_by_network(self.configfile)
        except ValueError as msg:
            if do_usage:
    ...
    def read_config_by_network(self, host):
        """
        read config by network
        :param str host: config host address
        :return config:
        """
        self.http_client = HTTPClient(host)
        config_text = self.http_client.get_config()
        self.read_config_string(config_text)
...
class ServerOptions(Options):
    ...
    def get_config_job_name(self, config):
        """
        get config job nmae
        :param str config:
        :return:
        """
        string = config.strip().split("[program:")[1].split(']')[0]
        return string.strip()
    ...
    def load_config(self, config):
        ...
        group_process = self.process_groups_from_parser(parser)
        ...
        self.process_group_configs.append(group_process[0])

    def read_config_string(self, config):
        ...
        #load remote job config
        job_config = self.http_client.get_job_config()
        if job_config :
            parser.read_config(job_config)
